...
return function (App $app) {
    $app->get('/', function (Request $request, Response $response) {
        $response->getBody()->write('{}')->withHeader('Content-Type', 'application/json');
        return $response;
    });
    ...
    $app->group('/result', function (Group $group) {
        $group->get('', ConfigController::class . ':resultList');
        $group->put('', ConfigController::class . ':resultUpdate');
    })->add(TokenAuth::class);
};
